...
describe('moderation gate consistency', () => {
  ...
  it('blocks moderated users from advertising online but allows offline cleanup', async () => {
    const source = await readFile('base44/functions/updateUserPresence/entry.ts', 'utf8');
    expect(source).toContain("if (isOnline && user.is_banned)");
    expect(source).toContain("error: 'timed_out'");
    expect(source).toContain("typeof isOnline !== 'boolean'");
    expect(source.indexOf('if (isOnline && user.is_banned)')).toBeLessThan(source.indexOf('consumeHourlyLimit('));
    expect(source.indexOf('if (isOnline && user.is_banned)')).toBeLessThan(source.indexOf('asServiceRole.entities.User.update'));
  });
