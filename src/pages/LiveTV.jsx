    ...
    <main className="mx-auto w-full max-w-7xl space-y-5 p-4 pb-24 sm:p-6">
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
        <section className="overflow-hidden rounded-2xl border bg-black">
          {selected ? (
            <div>
              <video ref={videoRef} key={selected.url} src={/\.m3u8(?:$|[?#])/i.test(selected.url) ? undefined : selected.url} controls autoPlay playsInline preload="auto" className="aspect-video w-full bg-black" onCanPlay={(e) => { setError(""); e.currentTarget.play().catch(() => {}); }} onError={() => setError("This channel could not play in the browser. Try another channel; the source may be offline or may block browser playback.")} />
