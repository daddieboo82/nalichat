...
const stageLooks = {
  neon: 'from-fuchsia-950 via-slate-950 to-purple-950 border-fuchsia-400/30',
  gold: 'from-amber-950 via-slate-950 to-stone-950 border-amber-400/30',
  ice: 'from-cyan-950 via-slate-950 to-blue-950 border-cyan-400/30',
};
...
export default function BattleRoom() {
  ...
  return <div className="h-full overflow-y-auto bg-slate-950 px-4 py-6 pb-28 text-white">
    <div className="mx-auto max-w-6xl">
      ...
      <header className={`relative mt-4 overflow-hidden rounded-3xl border bg-gradient-to-br ${stageLooks[battle?.stage_theme] || stageLooks.neon} p-5 text-center shadow-[0_0_50px_rgba(217,70,239,.12)] sm:p-8`}>
        ...
        <div className="relative"><p className="text-xs font-bold uppercase tracking-[.25em] text-fuchsia-200">NaliChat battle arena</p><h1 className="mt-2 text-2xl font-black sm:text-4xl">{battle?.title || 'Live battle'}</h1>
          ...
          <div className="mt-6 grid grid-cols-[1fr_auto_1fr] items-center gap-2 text-sm font-bold sm:text-xl"><span className="truncate text-fuchsia-200">{battle?.creator_name || 'Artist one'}</span><span className="rounded-full border border-white/30 bg-white/10 px-3 py-2 text-xs font-black text-amber-300 sm:text-base">VS</span><span className="truncate text-amber-200">{battle?.opponent_name || 'Artist two'}</span></div>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        {[['creator', battle?.creator_id, battle?.creator_name || 'Artist one', battle?.creator_track_url, battle?.creator_track_name],
          ['opponent', battle?.opponent_id, battle?.opponent_name || 'Artist two', battle?.opponent_track_url, battle?.opponent_track_name]].map(([side, id, name, url, track]) =>
