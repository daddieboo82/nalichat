...
describe('NaliBase mobile recent storefront origin',()=>{
 it('resolves a world for recent storefront navigation',async()=>{ const s=await read('components/navigation/RecentlyVisited.jsx'); expect(s).toContain('resolveWorldForLocation(path)'); expect(s).toContain("onNavigate(path, world?.id || '')"); });
