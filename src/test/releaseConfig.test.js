...
describe('release configuration', () => {
  ...
  it('keeps Studio AI raw entity access read-only and exposes only validated automation tools', async () => {
    const agent = await readJson('base44/agents/studio_ai.jsonc');
    ...
    expect(agent.tool_configs.some((entry) => entry.function_name === 'suggestTrackTags')).toBe(true);
  });
  ...
  it('uses one canonical public origin across install and desktop surfaces', async () => {
    ...
    expect(download).toContain("const WINDOWS_DOWNLOAD_URL = 'https://nalichat-downloads.netlify.app/downloads/NaliChat-Portable.exe';");
  ...
  it('keeps iPhone mobile auth actions and DM navigation reachable', async () => {
    ...
    expect(messages).toContain('className="relative h-full max-h-full min-h-0');
...
describe('contact mutation response contracts', () => {
  it('only refreshes contact state after confirmed add/remove mutations', async () => {
    ...
    expect(source).toContain('res?.data?.contactId !== contactId');
