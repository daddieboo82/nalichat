...
describe('NaliBase Central Plaza spatial map',()=>{
 it('grounds authenticated users in the plaza before the six mall entrances',async()=>{ const s=await read('pages/Home.jsx'); expect(s).toContain('You are here · Central Plaza'); expect(s).toContain('aria-label="NaliBase Central Plaza map"'); expect(s).toContain('const PLAZA_WORLDS = WORLD_ORDER.map'); expect(s).toContain('path: WORLD_CONTEXTS[id].path'); });
});
