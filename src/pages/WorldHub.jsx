...
import { ArrowLeft, ArrowRight, MessageSquare, Music, Compass, FolderKanban, Video, Trophy, Users, UserPlus, Mic, Wand2, ListMusic, BarChart3, FolderOpen, Upload, Images, Film, Swords, Medal, Rocket, UserRound, Map, Sparkles } from 'lucide-react';
...
export default function WorldHub(){
  ...
  return <>
    <AnimatePresence>{splash&&<WorldSplash key={worldId} world={world} onComplete={()=>setSplash(false)}/>}</AnimatePresence>
    <main className="relative min-h-[100dvh] overflow-hidden bg-black px-4 pb-28 pt-20 sm:px-6 lg:px-8">
      ...
      <div className="relative mx-auto max-w-7xl">
        ...
        <section className="relative py-10 sm:py-16">
          <div className="mb-8"><p className="text-xs font-black uppercase tracking-[.28em] text-white/40">Explore the environment</p><h2 className="mt-2 font-heading text-3xl font-black sm:text-5xl">Choose your next destination.</h2></div>
          <div className="grid gap-5 md:grid-cols-2">{world.districts.map(([name,description,path,Icon],index)=><motion.div key={name} initial={{opacity:0,y:24}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:index*.06}} whileHover={{y:-7,scale:1.01}}>
            <Link to={path} onClick={()=>trackProductEvent('post_login_action',{source:`${worldId}_world`,action:`open_${name.toLowerCase().replace(/\s+/g,'_')}`})} className="group relative flex min-h-64 overflow-hidden rounded-[2rem] border border-white/10 bg-white/[.055] p-7 shadow-2xl backdrop-blur-2xl transition hover:border-white/25">
              ...
              <div className="relative z-10 flex w-full flex-col"><div className="flex items-start justify-between"><div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-white/15 bg-black/30"><Icon className="h-7 w-7"/></div><ArrowRight className="h-6 w-6 text-white/35 transition group-hover:translate-x-2 group-hover:text-white"/></div><div className="mt-auto pt-12"><p className="text-[10px] font-black uppercase tracking-[.22em] text-white/35">District {String(index+1).padStart(2,'0')}</p><h3 className="mt-2 font-heading text-3xl font-black">{name}</h3><p className="mt-2 max-w-md text-sm leading-relaxed text-white/55">{description}</p></div></div>
            </Link>
          </motion.div>)}</div>
        </section>
        <section className="mt-12 overflow-hidden rounded-[2.5rem] border border-white/10 bg-white/[.04] p-8 text-center backdrop-blur-2xl sm:p-12"><Sparkles className="mx-auto h-7 w-7 text-white/60"/><p className="mt-4 text-xs font-black uppercase tracking-[.3em] text-white/40">NaliBase Portal Network</p><h2 className="mx-auto mt-3 max-w-3xl font-heading text-3xl font-black sm:text-5xl">This is one world inside a much larger universe.</h2><p className="mx-auto mt-4 max-w-2xl text-sm text-white/55">Your identity, projects and creative momentum move with you. Return to the Hub whenever you want to enter another environment.</p><Link to="/" className="mt-7 inline-flex items-center gap-2 rounded-2xl bg-white px-6 py-3 font-bold text-black">Return to NaliBase <ArrowRight className="h-4 w-4"/></Link></section>
      </div>
    </main>
  </>;
}
