...
describe('project invite membership serialization', () => {
  it('serializes membership mutations with stale-lock recovery', async () => {
    ...
    expect(source).toContain('MEMBERSHIP_LOCK_TTL_MS = 5 * 60 * 1000');
    expect(source).toContain('acquireMembershipLock');
    expect(source).toContain('ProjectMembershipLock.create');
    expect(source).toContain('ProjectMembershipLock.get');
    expect(source).toContain('ProjectMembershipLock.delete(lockId)');
