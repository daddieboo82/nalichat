...
export async function acquireAccountDeletionLock(
  ...
  try {
    ...
    await entities.AccountDeletionLock.delete(id);
    try {
      await create();
      return id;
    } catch (retryError) {
...
export async function releaseAccountDeletionLock(
  ...
  try {
    await entities.AccountDeletionLock.delete(lockId);
    return true;
  } catch (error) {
    console.error('Failed to release account deletion lock:', { lockId, error });
    return false;
  }
}
