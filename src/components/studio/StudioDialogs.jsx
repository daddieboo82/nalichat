...
import { INSTRUMENTS, MIDI_CHANNELS, getInstrument } from '@/lib/instruments';
...
export default function StudioDialogs({
  ...
  const isInstrumentTrack = newTrackType === 'midi' || newTrackType === 'instrument';
  return (
    <>
            ...
            {isInstrumentTrack && (
              <>
                <div className="space-y-2">
                  <label className="text-sm font-medium">Instrument / Plugin</label>
                  <Select value={newTrackInstrument} onValueChange={setNewTrackInstrument}>
                  ...
                  <p className="text-[11px] text-muted-foreground">{getInstrument(newTrackInstrument).desc}</p>
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium">MIDI Channel</label>
                  <Select value={newTrackMidiChannel} onValueChange={setNewTrackMidiChannel}>
                    <SelectTrigger><SelectValue placeholder="Select channel" /></SelectTrigger>
                    <SelectContent>
                      {MIDI_CHANNELS.map(ch => (
                        <SelectItem key={ch} value={ch}>{ch === 'all' ? 'All' : `Channel ${ch}`}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
