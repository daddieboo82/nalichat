...
describe('NaliBase CREATE notification identity',()=>{
 ...
 it('preserves NaliBase privacy branding for locked CONNECT messages',async()=>{ const s=await read('base44/functions/notifyOnMessage/entry.ts'); expect(s).toContain("isLockedChat ? 'NaliBase'"); });
});
