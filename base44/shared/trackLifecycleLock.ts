...
export async function acquireTrackLifecycleLock(
  ...
  try {
    ...
    await entities.TrackLifecycleLock.delete(id);
    try {
      await create();
      return id;
    } catch (retryError) {
...
export async function releaseTrackLifecycleLock(
  ...
  try {
    await entities.TrackLifecycleLock.delete(lockId);
    return true;
  } catch (error) {
    console.error('Failed to release track lifecycle lock:', { lockId, error });
    return false;
  }
}
