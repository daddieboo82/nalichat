...
import { MessageSquare, Mic, FileAudio, Users, ArrowRight, ShieldCheck } from "lucide-react";
...
const features = [
  { icon: MessageSquare, title: "Real-Time Creator Messaging", text: "Message artists, producers, and collaborators in direct or group conversations." },
  { icon: Mic, title: "Voice Notes for Fast Ideas", text: "Send voice notes when a melody, lyric, or production idea is easier to explain out loud." },
  { icon: FileAudio, title: "Share Audio & Files", text: "Keep demos, references, and project files close to the conversation that created them." },
  { icon: Users, title: "Built for Collaboration", text: "Move from conversation into a shared creator workflow without switching between disconnected tools." },
];

export default function CreatorMessagingLanding() {
  captureMarketingAttribution();

  useEffect(() => {
    const previousTitle = document.title;
    const description = "Real-time creator messaging for artists and music teams with voice notes, audio sharing, file sharing, and collaboration tools. Start free with NaliChat.";
    document.title = "Creator Messaging for Artists | NaliChat";
    ...
    <main className="min-h-[100dvh] bg-background text-foreground">
      <section className="mx-auto max-w-6xl px-4 pb-14 pt-[max(4rem,env(safe-area-inset-top))] sm:px-6 sm:pb-20">
        <div className="mx-auto max-w-4xl text-center">
          <p className="mb-4 text-sm font-bold uppercase tracking-[0.2em] text-primary">NaliChat creator messaging</p>
          <h1 className="font-heading text-4xl font-black tracking-tight sm:text-6xl">Real-Time Creator Messaging for Artists & Music Teams</h1>
          <p className="mx-auto mt-6 max-w-3xl text-lg leading-relaxed text-foreground/80 sm:text-xl">
            Message creators in real time, send voice notes, share audio and files, and keep collaboration connected to the conversation.
          </p>
        ...
        <section className="mt-10 rounded-3xl border border-border/70 bg-card/50 p-6 text-center sm:p-8">
          ...
          <h2 className="mt-3 text-2xl font-bold">Keep creator conversations connected</h2>
          <p className="mx-auto mt-3 max-w-2xl text-muted-foreground">NaliChat brings messaging, voice notes, audio sharing, files, and collaboration into one creator workspace.</p>
        </section>
