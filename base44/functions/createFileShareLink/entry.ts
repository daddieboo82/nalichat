...
Deno.serve(async (req) => {
  try {
    ...
    const projectLockId = file.project_id
      ? await acquireProjectMembershipLock(entities, file.project_id)
      : null;
    if (file.project_id && !projectLockId) {
    ...
    try {
      if (file.project_id) {
        ...
        if (
          currentFile.project_id !== file.project_id
          || !isBase44EntityId(currentFile.project_id)
        ) {
          return Response.json({ error: 'File project changed. Please retry.' }, { status: 409 });
        }
        file = currentFile;
