...
describe('NaliBase mobile recent storefronts',()=>{
 it('does not let Central Plaza consume a recent-storefront slot',async()=>{ const s=await read('components/navigation/RecentlyVisited.jsx'); expect(s).toContain('const EXCLUDE = ["/",'); expect(s).toContain('parsed.map(normalizeEntry).filter(entry => entry.path && entry.path !== "/")'); });
