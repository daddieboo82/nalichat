...
Deno.serve(async req => {
  try {
    ...
    const winners = new Map<string, { id: string; category: string; wins: number; points: number }>();
    for (let skip = 0; skip < 10000; skip += 200) {
      ...
      for (const b of rows) {
        ...
        const previous = winners.get(key) || { id: b.winner_id, category: b.category, wins: 0, points: 0 };
        previous.wins++;
        previous.points += b.winner_id === b.creator_id ? Number(b.creator_score || 0) : Number(b.opponent_score || 0);
        winners.set(key, previous);
      }
      if (rows.length < 200) break;
    }
    const rank = category => [...winners.values()].filter(w => w.category === category)
      .sort((a,b) => b.wins - a.wins || b.points - a.points).slice(0, 20);
    ...
    return Response.json({ success: true, rap: await decorate(rank('rap')), singing: await decorate(rank('singing')), scoring: 'Completed battle wins, then audience quality points.' });
  } catch (error) {
