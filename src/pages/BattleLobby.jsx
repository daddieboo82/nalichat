...
export default function BattleLobby() {
  ...
  return <div className="h-full overflow-y-auto bg-slate-950 px-4 py-8 pb-28 text-white"><div className="mx-auto max-w-5xl">
    ...
    <section className="mt-8"><h2 className="text-xl font-bold">Battle awards</h2><p className="mt-1 text-sm text-slate-400">Ranked by verified battle wins, then audience quality points. No award is shown until a battle is completed.</p><div className="mt-3 grid gap-3 sm:grid-cols-2">{[['Best Rapper', awards.rap], ['Best Singer', awards.singing]].map(([label, rows]) => <div key={label} className="rounded-xl border border-amber-400/20 bg-white/5 p-4"><h3 className="font-bold text-amber-300">{label}</h3>{rows.length ? <ol className="mt-2 space-y-2">{rows.slice(0,5).map((row, i) => <li key={row.id} className="text-sm">{i + 1}. {row.artist_name} · {row.wins} wins · {row.points} points</li>)}</ol> : <p className="mt-2 text-sm text-slate-400">No verified winner yet.</p>}</div>)}</div></section>
    <section className="mt-8"><h2 className="text-xl font-bold">Invitations</h2><div className="mt-3 grid gap-3 sm:grid-cols-2">{invites.filter(b => b.status !== 'cancelled').map(b =>
      <article key={b.id} className="rounded-xl border border-white/15 bg-white/5 p-4"><p className="font-bold">{b.title}</p><p className="text-sm capitalize text-slate-300">{b.category} · {b.status}</p>
        ...
        {b.status === 'invited' && <div className="mt-3 flex gap-2"><button disabled={busy === b.id} onClick={() => act(b.id,'accept')} className="rounded-lg bg-fuchsia-600 px-4 py-2 disabled:opacity-50">Accept battle</button><button disabled={busy === b.id} onClick={() => act(b.id,'decline')} className="rounded-lg border border-white/30 px-4 py-2 disabled:opacity-50">Decline</button></div>}
