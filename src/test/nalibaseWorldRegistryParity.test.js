...
describe('NaliBase world registry parity',()=>{
 ...
 it('keeps architecture and navigation context in sync',()=>{ expect(Object.keys(NALIBASE_ARCHITECTURE.worlds).sort()).toEqual([...WORLD_ORDER].sort()); for (const id of WORLD_ORDER) { expect(WORLD_CONTEXTS[id].path).toBe(`/world/${id}`); expect(WORLD_CONTEXTS[id].storefronts).toHaveLength(4); expect(NALIBASE_ARCHITECTURE.worlds[id]).toEqual(WORLD_CONTEXTS[id].storefronts.map(({label})=>label)); } });
});
