...
export default function LiveBattles() {
  ...
  return <div className="h-full overflow-y-auto bg-slate-950 text-white">
    <div className="mx-auto max-w-6xl px-4 py-8 pb-24">
      ...
      <div role="status" className="mt-6 flex gap-3 rounded-2xl border border-amber-400/40 bg-amber-400/10 p-4 text-sm text-amber-100"><AlertCircle className="h-5 w-5 shrink-0" /><span>Test your camera locally, upload a track, then invite another artist from the battle lobby. Audience scoring opens after both performers publish media in the live room. Camera preview stays on this device.</span></div>
      <section aria-label="Battle stage preview" className={`relative mt-6 overflow-hidden rounded-3xl border border-white/15 bg-gradient-to-br ${themes[theme]} p-4 shadow-2xl sm:p-8`}>
        ...
        <div className="relative">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="flex aspect-video items-center justify-center rounded-2xl border border-white/20 bg-black/60 text-center text-slate-300"><div><Mic className="mx-auto mb-2 h-10 w-10" />Opponent stage<br /><span className="text-xs">Appears when your invited artist joins</span></div></div>
          </div>
          <div className="mt-5 grid grid-cols-3 gap-2 text-center text-xs sm:text-sm"><div className="rounded-xl bg-black/30 p-3">Live round<br /><strong>Starts from the lobby</strong></div><div className="rounded-xl bg-black/30 p-3">Crowd energy<br /><strong>Awaiting audience</strong></div><div className="rounded-xl bg-black/30 p-3">Score reveal<br /><strong>No result yet</strong></div></div>
        </div>
      </section>
      ...
      <section className="mt-6 grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-white/10 p-4"><Users className="mb-2 text-fuchsia-300" /><strong>Live audience</strong><p className="text-sm text-slate-400">Watch performers and send reactions during live battles.</p></div><div className="rounded-xl border border-white/10 p-4"><Volume2 className="mb-2 text-fuchsia-300" /><strong>Fair scoring</strong><p className="text-sm text-slate-400">One scorecard per viewer, rating both artists on three musical criteria.</p></div><div className="rounded-xl border border-white/10 p-4"><Trophy className="mb-2 text-fuchsia-300" /><strong>Artist awards</strong><p className="text-sm text-slate-400">Best Rapper and Best Singer will reflect completed battles.</p></div></section>
      {!consent && track && <p className="mt-3 text-xs text-amber-300">Confirm track rights before uploading for a public battle.</p>}
    </div>
  </div>;
}